  ...
  #Record.find_all_by_asin("B007DKN4GC").each do |record|
    ...
    if image_link == nil
      ...
      image_search.css(".product").each do |prod|
        ...
        if prod.css(".productImage").empty? == false && (prod_artist.include?(record_artist) || record_artist.include?(prod_artist)) && (prod_title.include?(record_title) || Levenshtein.distance(prod_title,record_title) <= 2 || record_title.include?(prod_title))
